  ...
  useEffect(() => {
    ...
    const loadCredentials = async () => {
      try {
        ...
        const savedCountryCode = (await AsyncStorage.getItem(
          "countryCode"
        )) as CountryCode;
  ...
  const onSubmit: SubmitHandler<FormValues> = async (data) => {
    ...
    if (loginMethod === "phone") {
      ...
      formattedInput =
        ...
        `${getCountryCallingCode(countryCode)}${data.emailOrPhone}`;
    } else {
  ...
  useEffect(() => {
    ...
    if (signInResp.loading === false) {
      ...
      if (signInResp.success) {
        ...
        // newModal.visible = true;
        newModal.message = `Sign in failed: ${
          signInResp.error || "Unknown error"
        }`;
      }
    } else {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.checkboxContainer}>
        ...
        >
          <Ionicons
            name={rememberPassword ? "checkbox-outline" : "square-outline"}
            size={20}
            color="#1D6AFF"
          />
        </TouchableOpacity>
