    ...
    <Animated.View style={containerStyles}>
      ...
      >
        ...
        <View style={[combineStyles(GlobalStyles, 'padding_t_sm', 'padding_b_sm', 'margin_b_sm'), { backgroundColor: 'rgba(255, 255, 255, 0.1)' }]}>
          <MenuContainer>
            <MenuItem title="Orders" icon="cart-outline" onPress={() => router.push('/(seller)/seller')} />
            <MenuItem title="Inventory" icon="clipboard-list-outline" onPress={() => router.push('/(seller)/seller')} />
            <MenuItem title="Earnings" icon="currency-usd" onPress={() => router.push('/(seller)/seller')} />
          </MenuContainer>
        </View>

        <MenuContainer>
          <MenuItem title="Switch to Buyer" onPress={() => {
            AsyncStorage.setItem('currentMode', 'customer');
            router.push('/(customer)')
            }} />
          <MenuItem title="Help & Support" onPress={() => router.push('/(seller)/profile')} />
        </MenuContainer>
