...
interface Compatibility3 {
  selectedCompatibilities: any;
  setCompatibilities: (value: any) => void;
}
const CompatibilityCompV3: React.FC<Compatibility3> = ({
  ...
  const { data: compatibleVehicles, isLoading: compatibleVehiclesLoading } =
    useFetchVehicleById({
      car_ids: [],
    });
  ...
  React.useEffect(() => {
    if (selectedCompatibilities.length > 0) {
      ...
      selectedCompatibilities.forEach((item: any) => {
        ...
        engineSet.add(
          `${item.vehicleDetails.typeName} - ${item.vehicleDetails.powerHpFrom}HP`
        ); // Engine
      });
  ...
  const handleSelectCompatibility = (item: any) => {
    const isSelected = selectedCompatibilities.some(
      (compatibility: any) =>
        compatibility.vehicleDetails.carId === item.vehicleDetails.carId
    );

    if (isSelected) {
      // Remove from selected list
      setCompatibilities((prevCompatibilities: any) =>
        prevCompatibilities.filter(
          (compatibility: any) =>
            compatibility.vehicleDetails.carId !== item.vehicleDetails.carId
        )
      );
                  ...
                  >
                    ...
                    <View style={styles.radioButtonContainer}>
                      <View
                        style={[
                          styles.radioButton,
                          isSelected(item) && styles.radioButtonSelected,
                        ]}
                      />
                    </View>
...
export default CompatibilityCompV3;
