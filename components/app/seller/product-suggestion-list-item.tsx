...
const ProductSuggestionItem: React.FC<any> = ({ item, setIsVisible }) => {
  const productImage = item?.images?.big
    ? { uri: item.images.big }
    ...
    >
      <View
        style={[
          ...
          { backgroundColor: "#FFF4DE", overflow: "hidden" },
        ]}
      >
        {/* Product Image */}
        <Image
        ...
        <View style={combineStyles(GlobalStyles, "padding_x_sm")}>
          ...
          <Text
            style={[
              combineStyles(GlobalStyles, "text_2xl", "font_medium"),
              { flex: 1, width: "75%", marginTop: 10 },
            ]}
          >
            {item.Description}
          </Text>
          ...
          >
            ...
            <Text style={combineStyles(GlobalStyles, "margin_l_xs")}>
              {item.mfrName}
            </Text>
          </View>
        </View>
